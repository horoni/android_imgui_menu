...
pub unsafe fn flush_cache(addr: *const u32, size: usize) {
...
unsafe fn make_rwx(addr: *mut u32, size: usize) -> i32 {
	let sys_page_size = sysconf(_SC_PAGE_SIZE) as usize;
	let page_addr = (addr as usize & !(sys_page_size - 1)) as *mut c_void;
	let page_size = if (addr as usize + size) & (sys_page_size - 1) != (addr as usize) & (sys_page_size - 1) {
		((size + sys_page_size - 1) & !(sys_page_size - 1)) + sys_page_size
	} else {
		(size + sys_page_size - 1) & !(sys_page_size - 1)
	};
...
pub unsafe fn init_hook_pool() {
	let pool_ptr = std::ptr::addr_of_mut!(INSNS_POOL) as *mut u32;
	make_rwx(
		pool_ptr,
